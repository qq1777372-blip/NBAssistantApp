...
struct NativeSycmView: View {
    ...
    var body: some View {
        List {
            ...
            Section { Picker("店铺范围", selection: $selectedShop) { Text("全部店铺（\(shops.count)）").tag(""); ForEach(shops) { Text($0.shopName).tag($0.shopID) } }; Picker("数据周期", selection: $period) { ForEach(Array(periods.enumerated()), id: \.offset) { _, item in Text(item.1).tag(item.0) } }.pickerStyle(.segmented).onChange(of: period) { _ in Task { await load(force: true) } } }
        ...
        }.navigationTitle("生意参谋")
            .task(id: scenePhase) {
                guard scenePhase == .active else { return }
                await load(force: true)
                await refreshAtNextDay()
            }
    ...
    private func load(force: Bool = false) async {
        do {
            let refresh = force ? "true" : "false"
            let path = "api/sycm/latest?period=\(period)&date=\(currentDayKey)&refresh=\(refresh)"
            async let shopRequest: [SycmShop] = session.get(path)
            async let deviceRequest: [SycmDevice] = session.get("api/sycm/collector-devices")
            (shops, devices) = try await (shopRequest, deviceRequest)
            error = nil
        } catch {
