...
private struct PeerShopForm: View {
    @EnvironmentObject private var session: NativeSession; @Environment(\.dismiss) private var dismiss; let item: PeerShopRecord?; let onSave: () async -> Void; @State private var name: String; @State private var url: String; @State private var remark: String; @State private var saving = false; @State private var error: String?
    ...
    var body: some View { NavigationStack { Form { if let error { Text(error).foregroundStyle(.red) }; TextField("店铺名称", text: $name); TextField("店铺链接", text: $url); TextField("备注", text: $remark, axis: .vertical).lineLimit(4...8) }.navigationTitle(item == nil ? "新增同行店铺" : "编辑同行店铺").toolbar { ToolbarItem(placement: .cancellationAction) { Button("取消") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("保存") { Task { await save() } }.disabled(saving || name.isEmpty) } } } }
...
private struct LicenseRecordForm: View {
    @EnvironmentObject private var session: NativeSession; @Environment(\.dismiss) private var dismiss; let item: LicenseRecordItem?; let onSave: () async -> Void; @State private var subject: String; @State private var code: String; @State private var legal: String; @State private var issue: String; @State private var expiry: String; @State private var remark: String; @State private var saving = false; @State private var error: String?
    ...
    var body: some View { NavigationStack { Form { if let error { Text(error).foregroundStyle(.red) }; TextField("主体名称", text: $subject); TextField("统一信用代码", text: $code); TextField("法定代表人", text: $legal); TextField("签发日期", text: $issue); TextField("到期日期", text: $expiry); TextField("备注", text: $remark, axis: .vertical).lineLimit(4...8) }.navigationTitle(item == nil ? "新增执照" : "编辑执照").toolbar { ToolbarItem(placement: .cancellationAction) { Button("取消") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("保存") { Task { await save() } }.disabled(saving || subject.isEmpty || code.isEmpty) } } } }
