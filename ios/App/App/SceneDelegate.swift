...
private struct LicenseCreateForm: View {
    ...
    private func save() async { saving = true; defer { saving = false }; do { let _: [LicenseRecord] = try await session.send("license-admin/licenses", method: "POST", body: ["plan_name": plan, "count": count, "duration_days": days, "max_devices": devices, "note": note, "feature_flags": ["pro": true]]); await onSave(); dismiss() } catch { error = session.message(for: error) } }
}
...
private struct NativeWarehouseView: View {
    ...
    var body: some View {
        List {
            ...
            case .outbound: ForEach(outbound.filter { matches("\($0.orderNo) \($0.warehouseName) \($0.trackingNo ?? "")") }) { row in WarehouseTextRow(title: row.orderNo, detail: "\(row.warehouseName) · \(lineSummary(row.items))", status: outboundStatus(row.status)).swipeActions { if let next = nextStatus(row.status) { Button("推进") { Task { await setStatus(row, next) } }.tint(.blue) }; if !["shipped","cancelled"].contains(row.status) { Button("取消", role: .destructive) { Task { await setStatus(row, "cancelled") } } } } }
